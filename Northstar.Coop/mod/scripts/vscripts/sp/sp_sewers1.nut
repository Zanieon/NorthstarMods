...
void function SewerSplit_GateThink( entity player )
{
	...
	if ( !IsValid( GetProtagonist().GetPetTitan() ) )
		CreatePetTitanAtOrigin( GetProtagonist(), button.GetOrigin() + < 400,0,-20 >, <0,0,0> )
	
	GetProtagonist().GetPetTitan().SetInvulnerable()
}
...
void function StartPoint_SewerArena( entity player )
{
	...
	waitthread PlayerConversation( "BT_Reunite", player )
...
void function KaneArena_PickupHelmetSequence( entity player )
{
	...
	foreach( p in GetPlayerArray() )
		thread KaneArena_HighlightKaneObjective( p )
	...
	entity btOwner
	foreach( entity titan in GetTitanArrayOfTeam( TEAM_MILITIA ) )
	{
		if( GetTitanCharacterName( titan ) == "bt" )
			btOwner = titan.GetBossPlayer()
	}
	foreach( entity titanPlayer in GetPlayerArray() )
	{
		if( GetTitanCharacterName( titanPlayer ) == "bt" )
			btOwner = titanPlayer
	}
	
	if( IsValid( btOwner ) )
		thread KaneArena_DisembarkNag( btOwner )
...
void function KaneArena_PickupHelmetThink( entity player )
{
	...
	while( true )
	{
		entity playerActivator = expect entity( useDummy.WaitSignal( "OnPlayerUse" ).player )
...
void function KaneArena_BTMovesOutOfTheWay( entity player )
{
	...
	if ( !IsValid( bt ) || GetTitanCharacterName( bt ) != "bt" )
		return
