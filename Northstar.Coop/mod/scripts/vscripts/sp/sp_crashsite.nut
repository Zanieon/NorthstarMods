...
void function BTIntro_Player( entity player )
{
	...
	waitthread Coop_SyncCutsceneToProtagonist( player, OGVictorySequence, animRef )
...
void function BliskIntro_Music( entity buddyTitan, entity player )
{
	...
	SetGlobalNetInt( "nighttimeAmbient", -1 )
	foreach( p in GetPlayerArray() )
		Remote_CallFunction_Replay( p, "ServerCallback_NighttimeAmbient", -1 )
}
...
void function BliskIntro_Player( entity player )
{
	...
	waitthread Coop_SyncCutsceneToProtagonist( player, OGDefeatedSequence, animRef )
...
void function StartPoint_Setup_FamilyPhoto( entity player )
{
	foreach( entity p in GetPlayerArray() )
	{
		...
		// no grunt chetter that isn't custom, will get reset at the "waking up"
		SetPlayerForcedDialogueOnly( p, true )
		...
		// remove hud for grunt
		Remote_CallFunction_Replay( p, "ServerCallback_HideHudIcons" )
	}
...
void function FamilyPhoto_Player( entity player )
{
	...
	waitthread Coop_SyncCutsceneToProtagonist( player, FamilyPhotoSequence, animRef )
...
//	thread PlayAnimTeleport( rock, "stone_wilds_prowler_encounter", animRef )
	waitthread Coop_SyncCutsceneToProtagonist( player, ProwlerSequence, animRef )
...
//	rock.Destroy()
	...
	foreach( p in GetPlayerArray() )
	{
		p.SetPlayerNetBool( "shouldShowWeaponFlyout", false )
		...
		p.MovementEnable()
		ShowName( p )
		p.Server_TurnOffhandWeaponsDisabledOff()
	}

	#if CONSOLE_PROG
	// Clear DVS override started in bt_intro
	foreach( p in GetPlayerArray() )
		DisableDVSOverride( p )
	#endif

	// slow player speed down for a short while
	foreach( p in GetPlayerArray() )
		thread LerpPlayerSpeed( p, 0.35, 1.0, 4.0 )

	// StatusEffect_Stop( player, file.statusEffect_turnSlow )
	foreach( p in GetPlayerArray() )
		StatusEffect_AddTimed( p, eStatusEffect.turn_slow, 0.3, 4, 4 )

	wait 2 // disable flyouts
	foreach( p in GetPlayerArray() )
		Remote_CallFunction_NonReplay( p, "ServerCallback_LevelIntroText" )

	wait 5
	foreach( p in GetPlayerArray() )
		p.SetPlayerNetBool( "shouldShowWeaponFlyout", true )
}
...
void function StartPoint_Grave( entity player )
{
	...
	foreach( p in GetPlayerArray() )
	{
		...
		thread DisplayLogbookHint( p, 4.0 )
		ScreenFade( p, 0, 0, 0, 255, 10, 5, FFADE_IN | FFADE_PURGE ) // fade out the black screen
	}
	wait 4.5
	...
	{
		p.UnfreezeControlsOnServer()
		p.SetPlayerSettingsWithMods( DEFAULT_PILOT_SETTINGS, [ "disable_doublejump" ] )
		SyncedMelee_Enable( p )
		p.Server_TurnOffhandWeaponsDisabledOff()
	}
...
void function StartPoint_Skip_Grave( entity player )
{
	...
	foreach( p in GetPlayerArray() )
	{
		p.SetExtraWeaponMods( [] )
		p.SetPlayerNetBool( "hideHudIcons", false )
		Remote_CallFunction_Replay( p, "ServerCallback_HideHudIcons" )
		for( int x = 0; x < 5; x++ )
			Remote_CallFunction_Replay( p, "ServerCallback_ShowHudIcon", x )
		p.SetPlayerSettingsWithMods( DEFAULT_PILOT_SETTINGS, [ "disable_doublejump" ] )
		p.GiveOffhandWeapon( "mp_ability_cloak", OFFHAND_SPECIAL )
		SyncedMelee_Enable( p )
	}
}
...
void function Grave_DonHelmet( entity player )
{
	...
	foreach( p in GetPlayerArray() )
	{
		p.SetMoveSpeedScale( 0.4 )
		SetPlayerSprint( p, false )
	}
...
void function JumpKitCalibrationThread( entity player, float startProgress = 0.0 )
{
	...
	foreach( p in GetPlayerArray() )
	{
		AddPlayerMovementEventCallback( p, ePlayerMovementEvents.DOUBLE_JUMP, PlayerDoubleJumped )
		thread DisplayDoubleJumpHint( p )
	}
}
