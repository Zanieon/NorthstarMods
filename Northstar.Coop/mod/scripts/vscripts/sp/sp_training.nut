...
void function Training_PodIntro( entity player )
{
	...
	player.SetOrigin( GetProtagonist().GetOrigin() )
	player.EndSignal( "OnDestroy" )
	player.SetExtraWeaponMods( [ "low_ammo_disable" ] )
	SetWeaponHUDEnabled( player, false )
	...
	waitthread Coop_SyncCutsceneToProtagonist( player, playerSequence, pod )
...
void function StartGlitchScreenFX_Delayed( entity player, float screenFXTime, float delay )
{
	...
	foreach( p in GetPlayerArray() )
		Remote_CallFunction_Replay( p, "ScriptCallback_PodGlitch_PlayerScreenFX", screenFXTime )
}
...
void function Training_PodOutro( entity player )
{
	...
	foreach( entity p in GetPlayerArray() )
	{
		ScreenFade( p, 0, 0, 0, 255, 0, screenFadeWait, FFADE_OUT | FFADE_STAYOUT )
		Remote_CallFunction_Replay( p, "ScriptCallback_SimPodShutdownScreen", shutdownScreenWait )
	}
...
void function FancyTeleport_EffectsAndSound( entity player, vector teleportPos )
{
	...
	foreach( entity p in GetPlayerArray() )
		Remote_CallFunction_Replay( p, "ScriptCallback_PodTransition_PlayerScreenFX" )
