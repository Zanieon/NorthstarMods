...
void function IntroRadioDialogue( entity player )
{
	...
	waitthread PlayerConversation( "IntroRadioResponse", player )
...
void function StartPoint_LoadingDock( entity player )
{
	...
	waitthread PlayerConversation( "ControlPanelShortcut", player )
...
{
	if ( !IsValid( bt ) )
		bt = CreatePetTitanAtOrigin( GetProtagonist(), GetProtagonist().GetOrigin(), GetProtagonist().GetAngles() )
