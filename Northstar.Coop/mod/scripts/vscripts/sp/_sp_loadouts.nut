...
PilotLoadoutDef function GetPilotLoadoutForCurrentMapSP()
{
	...
	switch ( GetMapName() )
	{
		...
		case "sp_sewers1": // Reclamation
			...
			pilotLoadout.primaryMods 			= [ "extended_ammo","threat_scope" ]
		...
		case "sp_beacon_spoke2":
			...
			pilotLoadout.primaryMods 			= [ "pas_fast_reload","pas_fast_ads" ]
			pilotLoadout.secondary 				= "mp_weapon_wingman"
			pilotLoadout.ordnance 				= "mp_weapon_grenade_electric_smoke"
			pilotLoadout.special 				= "mp_ability_cloak"
			break
...
bool function ClientCommand_bt_loadout_select( entity player, array<string> args )
{
	...
	if( GetTitanCharacterName( player ) != "bt" )
	{
		SendHudMessage( player, "You are not piloting BT to swap loadouts!", -1, 0.4, 255, 255, 255, 255, 0.15, 3.0, 0.5 )
		return true
	}
